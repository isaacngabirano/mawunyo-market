...
export default {
  ...
  changePassword: (details) => {
    return new Promise(async (resolve, reject) => {
      ...
      if (user) {
        ...
        let status = await bcrypt
          .compare(details.currPass, user.password)
          .catch(() => {
            reject();
          });
  ...
  getCartItems: (userId) => {
    return new Promise(async (resolve, reject) => {
      ...
      if (amount && amount.length !== 0) {
        ...
        console.error("Invalid or undefined 'amount':", amount);
        amount = {
          _id: "",
          totalPrice: 0,
          totalDiscount: 0,
                    ...
                    $gte: [
                      {
                        $multiply: [
                          ...
                          parseInt(order.buyDetails.price),
                        ],
                      },
                      parseInt(discount.min),
                    ],
  ...
  AddAddress: (
    { name, number, pin, locality, address, city, state },
    userId
  ) => {
    return new Promise(async (resolve, reject) => {
      let saved = await db
