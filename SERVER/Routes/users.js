...
import trackProduct, {
  orderStatusControl,
} from "../ShipRocket/trackProduct.js";
...
router.get("/product/:slug/:proId", (req, res, next) => {
  if (req.params["proId"].length === 24) {
    product
      .getOneUserProduct(req.params.slug, req.params.proId)
      .then(async (item) => {
        if (product !== null) {
          let similar = await product
            .getSimilarProduct(item.categorySlug)
            .catch((err) => {
              console.log(err);
            });
          res.status(200).json({
...
router.get("/getReviews", async (req, res) => {
  ...
  product
    .getAllReviews(req.query.proId, 0, 5)
    .then((reviews) => {
      res.status(200).json({
        reviews: reviews,
...
router.get("/loadMoreReviews", async (req, res) => {
  ...
  let reviews = await product
    .getAllReviews(req.query.proId, parseInt(req.query.skip), 5)
    .catch(() => {
      res.status(500).json("err");
    });
...
router.get("/getLayouts", async (req, res) => {
  let sectionone = await layout
    .getSectionsCategory("sectionone")
    .catch((err) => {
      res.status(500).json("err");
    });
          ...
          .then(async (data) => {
            if (data) {
              ...
              getOTP((otp) => {
                if (otp) {
                  user
                    .insertOtp(req.body.email, otp, "signup", "user")
                    .then(async (done) => {
                      let mailDetails = {
                        from: process.env.MAIL_FROM,
                        to: req.body.email,
                    ...
                    user
                      ...
                      .then((response) => {
                        if (response) {
                          res
                            .status(200)
                            .json({ found: false, status: true, user: true });
                        } else {
                          res
                            .status(200)
                            .json({ found: false, status: true, user: false });
                        }
                      })
                      .catch((err) => {
                        res.status(500).json("err");
          ...
          .then(async (data) => {
            if (data) {
              ...
              getOTP((otp) => {
                if (otp) {
                  user
                    .insertOtp(req.body.email, otp, "forgot", "user")
                    .then(async (done) => {
                      let mailDetails = {
                        from: process.env.MAIL_FROM,
                        to: req.body.email,
...
router.get("/getWishlistItems", CheckUser, (req, res) => {
  user
    .getWishlistItems(req.query.userId)
    .then((result) => {
...
router.get("/getCartTotalPrice", (req, res) => {
  user
    .getCartTotalPrice(req.query.userId)
    .then((amount) => {
...
router.get("/getCartTotalPriceCheckout", CheckUser, async (req, res) => {
  ...
  user
    .getCartTotalPriceCheckout(req.query.userId, req.query.discount)
    .then((amount) => {
      res.status(200).json({
...
router.get("/findCupon", (req, res) => {
  product
    .findCupon(req.query.code.toUpperCase())
    .then((data) => {
      res.status(200).json(data);
    })
    .catch(() => {
...
router.post("/order-item-razorpay", async (req, res) => {
  ...
  let AddAddress = await user
    .AddAddress(details, razorpayRes.userId)
    .catch(() => {
  ...
  if (payStatus) {
    ...
    fetchPayment(razorpayRes.razorpay_payment_id, async (data) => {
      ...
      if (order["order"].type === "cart") {
        let orderItems = await user
          .getCartProduct4Order(
            {
        ...
        orderItems.order = await ShipRocketOrder(
          "Prepaid",
          orderItems.order,
          token
        ).catch(() => {
