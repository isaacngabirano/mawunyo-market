...
router.get("/getDashboard", CheckVendor, async (req, res) => {
  ...
  let orders = await vendor
    .getAllOrders({ search: "", skip: 0, vendorId: req.body.vendorId }, 10)
    .catch(() => {
      res.status(500).json("err");
    });
...
router.post("/sentOtpLogin", (req, res) => {
  req.body.email = req.body.email.toLowerCase();
  vendor
    .checkVendorAccept(req.body.email)
    .then((accept) => {
      if (accept) {
        vendor
          .checkOtp(req.body.email, "login", "vendor")
          .then((oldOtp) => {
            if (oldOtp) {
              let mailDetails = {
              ...
              getOTP((otp) => {
                if (otp) {
                  vendor
                    .insertOtp(req.body.email, otp, "login", "vendor")
                    .then(async (done) => {
                      let mailDetails = {
                        from: process.env.MAIL_FROM,
                        to: req.body.email,
...
router.post("/login", async (req, res) => {
  req.body.email = req.body.email.toLowerCase();
  vendor
    .getVendorAccepted(req.body.email)
    .then(async (vendorAccepted) => {
      if (vendorAccepted) {
        const match = await bcrypt.compare(req.body.password, vendorAccepted.password);
        if (match) {
...
router.post(
  "/addProduct",
  CheckVendor,
  uploader.products.array("images", 20),
  (req, res, next) => {
...
router.get("/getProducts", CheckVendor, async (req, res, next) => {
  if (req.query.search === undefined) {
    let proCount = await vendor
      .getProductVendorCount(req.query.vendorId)
      .catch((err) => {
        console.log(err);
      });
    ...
    vendor
      .getVendorProducts(skip, limit, req.query.vendorId)
      .then((data) => {
        response.data = data;
    ...
    let allProductCount = await vendor
      .getProductVendorCount(req.query.vendorId)
      .catch((err) => {
        console.log(err);
      });
    ...
    let proCount = await vendor
      .getProductCountVendorSearch(req.query.search, req.query.vendorId)
      .catch((err) => {
        console.log(err);
      });
...
router.get("/getOneProduct/:prodId", CheckVendor, (req, res) => {
  if (req.params["prodId"].length === 24) {
    vendor
      .getOneProduct(req.body.vendorId, req.params.prodId)
      .then((product) => {
...
router.put(
  ...
  (req, res) => {
    if (req.params["id"].length === 24) {
      ...
      vendor
        .updateProduct(data, req.query.vendorId)
        .then((succ) => {
          if (delImgs.length !== 0) {
